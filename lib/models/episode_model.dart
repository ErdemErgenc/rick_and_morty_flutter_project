...
class EpisodesModel {
  ...
  static Future<EpisodesModel> fromList(List results) async {
    if (results.isEmpty) {
      throw Exception('Results list is empty');
    }
    final info = InfoModel(count: results.length, pages: 1); // Example InfoModel initialization
    final episodes = results
        .map((e) => EpisodeModel.fromMap(e as Map<String, dynamic>))
        .toList();
    return EpisodesModel(info: info, episodes: episodes);
  }
}
